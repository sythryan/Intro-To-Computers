...
-- This program maintains the warehouse inventory of different pasta varieties
   ...
   Inventory_File_Name : constant String := "Records.Dat";

   -- Type for Description
   subtype Description_Type is String (1 .. 20);

   -- Types for sales
   subtype Num_Sales is Integer range 0 .. Integer'Last;   -- Number of sales
   subtype Sales_Total is Float range 0.0 .. Float'Last;   -- Total amount in pounds sold
   ...
   -- the amount of pasta sold from the inventory.
      ...
      Overall      : Pasta_Rec;        -- All data in one index
   ...
   begin
      ...
      Overall.Amt := Overall.Amt - Amount;
      Overall.Number_Of_Sales := Overall.Number_Of_Sales + 1;
      Overall.Total_Sales := Overall.Total_Sales + Sales_Total (Amount);
   ...
   -- of that variety to zero.
      ...
      Length        : Natural;        -- Length of the description
   ...
   begin
      ...
      Ada.Text_IO.New_Line (2);
      Ada.Text_IO.Put ("Enter a description ");

      Ada.Text_IO.Get (Item => Info (1 .. Length));
   ...
   begin
      ...
      -- Display the total
      Ada.Text_IO.Put ("ID #: ");
      Ada.Integer_Text_IO.Put (Integer (Pasta_Number), 1); -- Put pasta ID number
      Ada.Text_IO.New_Line;
      Ada.Text_IO.Put ("Description: ");
      Ada.Text_IO.Put_Line (Overall.Description);   -- Put pasta description
      Ada.Text_IO.Put ("The amount of that pasta in stock is ");
      Ada.Float_Text_IO.Put (Item => Overall.Amt, Fore => 1, Aft  => 1, Exp  => 0);
      Ada.Text_IO.Put_Line (" pounds.");
      Ada.Text_IO.Put ("Number of sales: ");
      Ada.Integer_Text_IO.Put (Item => Overall.Number_Of_Sales, Width => 1); -- Put number of sales
      Ada.Text_IO.New_Line;
      Ada.Text_IO.Put ("Total sales: ");
      Ada.Float_Text_IO.Put (Item => Overall.Total_Sales, Fore => 1, Aft => 1, Exp => 0); -- Put total
      Ada.Text_IO.Put_Line (" pounds.");                                                  -- Sales in pounds
   ...
   begin
      ...
      loop         -- added to the total
         ...
         Pasta_IO.Read (File => Pasta_File,  Item => Overall); -- amount of one pasta in stock
         Total := Total + Overall.Amt;
      end loop Sum_Loop;
      ...
      Ada.Text_IO.New_Line;
      Ada.Text_IO.Put ("Pasta #  ");                             -- Display totals table
      Ada.Text_IO.Put           ("Description");
      Ada.Text_IO.Put                       ("         Amount");
      Ada.Text_IO.Put                                       ("     Number of Sales");
      Ada.Text_IO.Put                                                            ("      Pounds Sold");
      Ada.Text_IO.New_Line (2);
--      for Index in Pasta_ID_Type'Range loop
         Ada.Integer_Text_IO.Put (Integer (Pasta_Number), 7);
         Ada.Text_IO.Put (Overall.Description);
         Ada.Float_Text_IO.Put (Item => Overall.Amt, Fore => 7, Aft  => 1, Exp  => 0);
         Ada.Integer_Text_IO.Put (Overall.Number_Of_Sales, 17);
         Ada.Float_Text_IO.Put (Item => Overall.Total_Sales, Fore => 10, Aft => 1, Exp => 0);
--      end loop;
      Ada.Text_IO.New_Line (2);
